...
test("parseRunningCopies: still reports a profile whose own name contains 'Helper'", async () => {
  const { parseRunningCopies } = await import("../src/commands/doctor.js");

  // Excluding helpers by name as well as by path would swallow this profile,
  // and a profile the check stays silent about is the failure it exists to
  // catch: doctor would print "all started through their launcher" while this
  // one sits on the shared default account.
  const copy = "/Users/x/Library/Application Support/claude-multiprofile/apps/Claude Helper.app";
  const found = parseRunningCopies(`  501 ${copy}/Contents/MacOS/Claude`, copy);
