...
export function parseRunningCopies(psOutput, copyPath) {
  ...
  for (const line of psOutput.split("\n")) {
    // Renderer/GPU helpers live in their own nested bundles and inherit the
    // parent's profile, so only the top-level process is worth reporting.
    if (!line.includes(needle) || line.includes("Helper")) continue;
